<template>
  <div class="p-8">
    <div class="mb-6">
      ...
      <h1 class="text-3xl font-bold text-gray-900">學生資料批次匯入</h1>
      <p class="text-gray-600 mt-2">透過 Excel 檔案批次匯入學生資料</p>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600">按照範本格式填寫學生資料</p>
            </div>
          </div>
          <div class="flex items-center space-x-3">
          ...
          <div class="bg-green-50 rounded-lg p-4 border border-green-200">
            <div class="flex items-center">
              ...
              <div>
                ...
                <p class="text-sm text-green-700">成功匯入 {{ importResult.imported }} 筆學生資料，無任何錯誤。</p>
              </div>
            </div>
          </div>
        </div>
...
const fieldDescriptions = [
  { name: '年級', description: '學生年級（國一=1，國二=2，國三=3，高一=4，高二=5，高三=6）', example: '1', required: true },
  { name: '班級（數字）', description: '班級數字代號', example: '1', required: true },
  { name: '班級（中文）', description: '班級中文名稱', example: '忠', required: true },
  { name: '座號', description: '學生座號', example: '1', required: true },
  { name: '學號', description: '學生學號（唯一識別）', example: 'A001', required: true },
  { name: '姓名', description: '學生姓名', example: '王小明', required: true },
  { name: '身分證字號', description: '學生身分證字號（用於登入驗證）', example: 'A123456789', required: true },
  { name: '條件一', description: '自訂條件欄位（可用於性別等篩選）', example: '1', required: false },
  { name: '條件二', description: '自訂條件欄位（可用於部別等篩選）', example: '2', required: false },
  { name: '條件三', description: '自訂條件欄位（可用於其他篩選）', example: '特殊', required: false }
]

// 方法
const goBack = () => {
  router.push(`/schools/${schoolId.value}/admin/students`)
}
...
const downloadTemplate = async () => {
  ...
  try {
    ...
    link.href = '/學生資料匯入範本.xlsx'
    link.download = '學生資料匯入範本.xlsx'
