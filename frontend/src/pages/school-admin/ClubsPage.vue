<template>
  <div class="p-8">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-900">社團管理</h1>
      <div class="flex space-x-4">
        <button
          @click="navigateTo('/school-admin/clubs/import')"
...
const addClub = async () => {
  ...
  try {
    await clubStore.createClub(schoolId.value, {
      ...
      club_number: newClub.value.club_number,
      name: newClub.value.name,
      teacher_name: newClub.value.teacher_name,
      description: newClub.value.description,
      max_members: newClub.value.max_members,
      location: newClub.value.location,
      rainy_day_location: newClub.value.rainy_day_location,
      note: newClub.value.note,
      condition1: newClub.value.condition1,
      condition2: newClub.value.condition2,
      condition3: newClub.value.condition3,
      teacher_email: newClub.value.teacher_email
    })
...
const editClubDetail = (club: Club) => {
  ...
  editClub.value = {
    ...
    rainy_day_location: club.rainy_day_location || '',
    note: club.note || '',
    condition1: club.condition1 || 0,
    condition2: club.condition2 || 0,
    condition3: club.condition3 || '',
    teacher_email: club.teacher_email || ''
  }
...
const updateClub = async () => {
  ...
  try {
    ...
    await clubStore.updateClub(schoolId.value, clubId, {
      ...
      club_number: editClub.value.club_number,
      name: editClub.value.name,
      teacher_name: editClub.value.teacher_name,
      description: editClub.value.description,
      max_members: editClub.value.max_members,
      location: editClub.value.location,
      rainy_day_location: editClub.value.rainy_day_location,
      note: editClub.value.note,
      condition1: editClub.value.condition1,
      condition2: editClub.value.condition2,
      condition3: editClub.value.condition3,
      teacher_email: editClub.value.teacher_email
    })
