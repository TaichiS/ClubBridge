...
- **根本原因**: 測試 setup (`let!`) 中預先建立的學生資料，其 `student_id` 與��試匯入的 Excel 檔案中的 `student_id` 重複，觸發了���型���唯一���驗證 (`uniqueness`)。
...
1.  **模型與多租戶**:
    ...
    - 在 `School` 模型中加入了 `has_many :clubs` ���聯。
...
4.  **全面的測試更新**:
    ...
    - 更新了 **所有** 現存的請求測試 (`request specs`)，為每一個 API 請求都加入了認證標頭，確保了整個系統在新的認證授權機制下依然能正常運作。
...
本次開發是專案的一個重要轉折點。我們建立了一個專業、可擴充的認證授權系統，為後續所有功能的開發提供了安全保障。雖然過程中有小插曲，但都順利解決，並再次強化了我們對新版 Rails 的掌握。
...
3.  **產生文件**:
    - 成功執行 `rake rswag:specs:swaggerize` 指令，產生了完整的 `swagger.yaml` OpenAPI 文件。
...
- **根本原因**: 對 `rswag` 的 DSL 語法理解有誤，錯誤地將 `path` 放在了 `describe` ��塊的內部或外部。
- **解決方案**: 仔細查閱 `rswag` 的文件和範例，確認了正確的語法結構：`describe` 作為最外層，`path` 在其內部，而 `run_test!` 則在 `response` 區塊內。
- **📝 **學習與預防**:
  > **在引入一個新的、複雜的 DSL (領域特定語言) 時，不要想當然耳。花時間閱讀官方文件中的「Hello World」等級範例，是避免反覆試錯、提升效率的最佳途徑。**
...
- **📝 **學習與預防**:
  > **RSpec 的載入順序很重要。對於需要特定設定的 gem (如 `rswag`)，確保它的設定輔助檔案 (`swagger_helper.rb`) 在測試檔案中被明確且優先地 `require`���是保證測試環境正確初始化���關鍵。**
